...
void Cartridge::printHeader(iNES_Header header) {
  printf("Cartridge program size is: %d\n\
        CHR ROM Size is: %d\n\
        ...
        Ignore mirroring: %d\n\
        ",
         header.prgSize, header.chrSize, (header.flag[0] & 0x01),
         (header.flag[0] & 0x02) >> 1, (header.flag[0] & 0x04) >> 2,
...
uint8_t Cartridge::readPRGAddress(unsigned short address) {
  ...
  case Mapper::UNROM: {
    return unrom->readAddress(address);
    break;
  }
  }
}
...
uint8_t Cartridge::readCHRAddress(unsigned short address) {
  ...
  case Mapper::UNROM: {
    return unrom->readCHRAddress(address);
    break;
  };
  }
}
...
void Cartridge::write(unsigned short address, uint8_t value) {
  ...
  case Mapper::UNROM: {
    unrom->writeAddress(address, value);
    break;
  }
  }
...
bool Cartridge::getMirroringMode() { return (header.flag[0] & 0x01); }