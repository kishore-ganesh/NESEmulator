  ...
  if (address >= 0x2000 && address <= 0x2FFF) {
    ...
    case Mirroring::VERTICAL: {
      unsigned short baseAddress =
          ((address < 0x2400) || (address >= 0x2800 && address < 0x2c00))
              ? 0x000
              : 0x400;
      unsigned short offset = (address % 0x400);
...
void PPU::fetchTile(int tileNumber) {
  ...
  SPDLOG_INFO("Base nametable addess: {0:x},  xScroll: {2:d}, yScroll: {3:d}",
              baseNameTableAddress, tileNumber, xscroll, yscroll);
  ...
  short baseAttributeTableAddress =
      baseNameTableAddress +
      0x3C0; // check this, make this only one memory acces
  ...
  // Multiplying by 16 since each pattern has two consecutive parts (The upper
  // part and the lower part)
  unsigned short patternAddress = nameTableEntry * 16 +
                                  basePatternTableAddress +
                                  (baseY % 8); // Should not be current scanline
...
void PPU::renderTile(TileInfo tileInfo) {
  ...
  for (int patternBit = 0; patternBit < 8; patternBit++) {
    unsigned short palleteAddress = baseAddress | (tileInfo.attribute << 2) |
                                    ((tileInfo.lowerPattern >> 7) << 1) |
                                    (tileInfo.upperPattern >> 7);
    bool isTransparent = false;
    if ((tileInfo.lowerPattern >> 7) == 0 &&
        (tileInfo.upperPattern >> 7) == 0) {
      // Should be mirror
      if (tileInfo.background) {
    ...
    if (!tileInfo.background && tileInfo.spriteIndex == 0 && !isTransparent &&
        x < 256 && !bgTransparency[x][currentScanline]) {
    ...
    if (tileInfo.background) {
      if ((x - xscroll % 8) >= 0) {
        ...
        // SPDLOG_INFO("Background x: {0:d}, y: {1:d} is: {2:x}", x-xscroll%8,
        // tileInfo.y, palleteAddress);
        setPixel(x - xscroll % 8, currentScanline, palletes[palleteIndex]);
      }
    } else {
      // SPDLOG_INFO("x: {0:d}, y: {1:d}, actualX: {2:d}", x, tileInfo.y,
      // tileInfo.x);
      if (x < 256) {
        if (!isTransparent &&
            (!tileInfo.priority || bgTransparency[x][currentScanline])) {
          SPDLOG_INFO("SPRITE {}");
...
bool PPU::shouldRender() { return renderFlag && currentScanline == 240; }
