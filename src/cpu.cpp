...
EdgeInterrupt *CPU::getNMIPointer() { return &NMI; }

bool CPU::captureInput() { return shouldCaptureInput; }
// Stop input captur
...
void CPU::printStatus() {
  ...
  SPDLOG_INFO("Flags: \n N: {0:d} \n Z: {1:d} \n  Carry: {2:d} \n Overflow: "
              ...
              getFlag(NEGATIVE), getFlag(ZERO), getFlag(CARRY),
...
void CPU::processInstruction(uint8_t instruction) {
  ...
  if ((instruction & 0x1F) == 0x10) {
    ...
    case 0x0:
      BRANCH(NEGATIVE, bit);
      SPDLOG_INFO("Branch if Negative: {0:b}", bit);
      break;
    case 0x1:
      BRANCH(INTEGER_OVERFLOW, bit);
      SPDLOG_INFO("Branch if Overflow: {0:b}", bit);
      break;
    case 0x2:
      BRANCH(CARRY, bit);
      SPDLOG_INFO("Branch if Carry: {0:b}", bit);
      break;
    case 0x3:
      BRANCH(ZERO, bit);
      SPDLOG_INFO("Branch if Zero: {0:b}", bit);
      break;
    }
  } else if (cc == 0x01) {
    ...
    case 0x0: {
      ...
      address = (readAddress((address + X + 1) & 0x00FF) << 8) |
                readAddress((address + X) & 0x00FF);
    ...
    case 0x4: {
      ...
      address = (readAddress(PC)) & 0x00FF;
      address =
          ((readAddress((address + 1) & 0x00FF) << 8) | readAddress(address)) +
          Y;
      break;
    }
    case 0x5: {
    ...
    if (cc == 2) {
      ...
      case 0x3:
        ...
        break;
      case 0x4:
        STX(address, accumulator);
        break;
      case 0x5:
        LDX(address);
        break;
      case 0x6:
        DEC(address, accumulator);
        break;
      case 0x7:
...
void CPU::JMP(unsigned short address) {
  if (!((address & 0x00FF) == 0x00FF)) {
    ...
    short jumpTo =
        (readAddress(address & 0xFF00) << 8) | (readAddress(address));
    PC = jumpTo - 1;
  }
