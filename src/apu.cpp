...
void APU::cycle() {
  ...
  unsigned short pulse1Output = 0, pulse2Output = 0, triangleOutput = 0,
                 dmcOutput = 0, noiseOutput = 0;
  ...
  // spdlog::info("PULSE 1 output is: {0:d}, PULSE 2 output is: {1:d}",
  // pulse1Output, pulse2Output);
  ...
  if (!(triangleOutput == 0 && dmcOutput == 0 && noiseOutput == 0)) {
    double tndDenominator = triangleOutput / 8227.0 + (noiseOutput / 12241.0) +
                            (dmcOutput / 22638.0);
...
void PulseGenerator::writeRegister(unsigned short address, uint8_t value) {
  ...
  case 0: {
    ...
    switch (dutyCycle) {
    case 0: {
      currentSequence = 0x80;
      break;
    }
    case 1: {
      currentSequence = 0x60;
      break;
    }
    case 2: {
      currentSequence = 0x78;
      break;
    }
    case 3: {
      currentSequence = 0x9F;
      break;
    }
    }
...
unsigned short PulseGenerator::cycle() {
  ...
  timer--;
  bool currentSequenceBit =
      (currentSequence >> (7 - currentSequenceIndex)) & 0x01;
